...
if generate_rec or st.session_state.status:
    st.session_state.status = True

if st.session_state.status:
    animes = get_recommendations(user_input, cos_sim, indices).reset_index(drop=True)
    animes.index += 1
    sort_order = st.radio(label='Sort by (descending):', options=['Similarity (0 - 1)', 'Year', 'Mean'], horizontal=True)

    if sort_order == 'Similarity (0 - 1)':
        for i in range(1,21):
            ...
            with st.expander(label=f'{title} - Similarity Score: {score}', expanded=True):
                ...
                with col1:
                    st.image(animes['medium_picture_url'].loc[i])
                with col2:
                    st.header(animes['title'].loc[i])
                    st.write('Alternative title(s):', str(animes['alternative_title(s)'].loc[i]))
                    st.write('Genre(s):', animes['genres'].loc[i])
                    st.write('Studio(s):', animes['studios'].loc[i])
                    st.write('Year released:', str(animes['year'].loc[i]))
                    st.write('Status:', animes['status'].loc[i])
                    st.write('Synopsis:', animes['synopsis'].loc[i])
    
    elif sort_order == 'Year':
        animes = get_recommendations(user_input, cos_sim, indices).sort_values(by='year', ascending=False).reset_index(drop=True)
        animes.index += 1
        for i in range(1,21):
            st.write(f'Recommendation {i}:')
            title = animes['title'].loc[i]
        ...
        animes = get_recommendations(user_input, cos_sim, indices).sort_values(by='mean', ascending=False).reset_index(drop=True)
        animes.index += 1
        for i in range(1,21):
            st.write(f'Recommendation {i}:')
            title = animes['title'].loc[i]
